...
export async function POST(req: Request) {
	try {
		...
		const astronautIdCreated = user.astronautId;

		const existingHome = await prisma.file.findFirst({
			where: { path: "home" },
		});
		if (!existingHome) {
			await prisma.file.create({
				data: {
					path: "home",
					isFolder: true,
					astronautId: astronautIdCreated,
				},
			});
		}

		const userHomePath = `home/${astronautIdCreated}`;
		const existingUserHome = await prisma.file.findFirst({
			where: { path: userHomePath, astronautId: astronautIdCreated },
		});
		if (!existingUserHome) {
			await prisma.file.create({
				data: {
